            ...
            {vitalSigns.respiratory_rate && (
              <div className="flex items-center gap-1">
                ...
                <span className="font-medium">
                  {vitalSigns.respiratory_rate} rpm
                </span>
              </div>
            )}
          </div>
...
const AssessmentDataDisplay: React.FC<{ data: string | AssessmentData }> = ({
  data,
}) => {
  ...
  if (typeof data === "string") {
    try {
      ...
      return (
        <div className="text-gray-700 text-sm whitespace-pre-line">{data}</div>
      );
    }
  ...
  if (assessmentData) {
    return (
      <div className="space-y-3">
        ...
        {assessmentData.differentials &&
          assessmentData.differentials.length > 0 && (
...
export const SOAPHistory: React.FC<SOAPHistoryProps> = ({
  petId,
  className,
}) => {
